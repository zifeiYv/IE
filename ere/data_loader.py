...
class ChineseAndPunctuationExtractor(object):
    ...
    def is_chinese_or_punct(self, char):
        if self.chinese_re.match(char):
            return True
        else:
            return False
...
class DuIEDataset(paddle.io.Dataset):
    ...
    def from_file(cls,
                  ...
                  max_length: Optional[int] = 512,
                  pad_to_max_length: Optional[bool] = None):
        assert os.path.isfile(file_path), f"{file_path} dose not exists or is not a file."

        label_map_path = os.path.join('./config_data', "predicate2id.json")
        assert os.path.isfile(label_map_path), f"{label_map_path} dose not exists or is not a file. "
